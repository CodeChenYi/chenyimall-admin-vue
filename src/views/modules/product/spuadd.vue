  ...
  <div>
    <el-row>
        ...
        <el-card class="box-card" style="width: 80%; margin: 20px auto">
          ...
          >
            ...
            <el-form-item label="选择品牌" prop="brandId">
              <brand-select :categoryPath.sync="categoryPath" @getBrandId="getBrandId"></brand-select>
            </el-form-item>
      ...
      <el-col :span="24" v-show="step == 1">
        <el-card class="box-card" style="width: 80%; margin: 20px auto">
          <el-tabs tab-position="top" style="width: 98%">
            <el-tab-pane
            ...
            >
              ...
              <el-form ref="form" :model="spu">
                <el-form-item
                  :label="attr.attrName"
                  v-for="(attr, aidx) in group.attrEntityList"
                  :key="attr.attrId"
                >
                  <el-input
...
export default {
  ...
  components: { CategoryCascader, MultiUpload, BrandSelect },
  props: {},
  data() {
    return {
      catPathSub: null,
  ...
  watch: {
    categoryPath() {
      if (this.categoryPath.length > 0) {
        console.log("---------------");
        this.spu.catalogId = this.categoryPath[this.categoryPath.length - 1];
        // this.$emit("catPath", this.categoryPath);
        // this.PubSub.publish('ccategoryPath', this.categoryPath)
      }
    },
    uploadImages(val) {
      //扩展每个skus里面的imgs选项
  ...
  methods: {
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attr/list/sale/${this.spu.catalogId}`
          ),
          method: "get",
        }).then(({ data }) => {
          this.dataResp.saleAttrs = data.list;
          console.log(this.dataResp.saleAttrs);
    ...
    showBaseAttrs() {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attrgroup/getAttrGroupWithAttrByCategoryId/${this.spu.catalogId}`
          ),
          method: "get",
          params: this.$http.adornParams({}),
        }).then(({ data }) => {
          ...
          data.list.forEach((item) => {
            let attrArray = [];
            if (item.attrEntityList != null) {
              item.attrEntityList.forEach((attr) => {
                attrArray.push({
                  attrId: attr.attrId,
                  attrValues: "",
          ...
          this.dataResp.attrGroups = data.list;
        });
      }
    },
